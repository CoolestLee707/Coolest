...
//        NSLog(@"%@ - %@ - %@",NSStringFromCGRect(rect),NSStringFromCGRect(screenRect),NSStringFromCGRect(intersectionRect));
    }
   
//    可以设置显示百分比，当前设置完全漏出才算曝光
    if (intersectionRect.size.height < rect.size.height) {
        return NO;
    }
