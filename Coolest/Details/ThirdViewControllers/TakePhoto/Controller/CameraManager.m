...
- (void)startUpdateCurrentOrientation{
    ...
    if([self.montionManager isDeviceMotionAvailable]) {
        [self.montionManager startAccelerometerUpdatesToQueue:[NSOperationQueue currentQueue] withHandler:^(CMAccelerometerData * _Nullable accelerometerData, NSError * _Nullable error) {
            //            ADLog(@"x : %f  y : %f  z: %f",accelerometerData.acceleration.x,accelerometerData.acceleration.y,accelerometerData.acceleration.z);
...
- (void)saveImageToLibrary:(UIImage *)image Finish:(void(^)(BOOL result))complete {
    ...
    self.saveBlock = complete;
    /**
     保存到相册
     @param image 图片对象
     @param self 成功方法绑定的target
...
{
    ADLog(@"image = %@, error = %@, contextInfo = %@", image, error, contextInfo);
    if(error){
        if (self.saveBlock) {
            self.saveBlock(NO);
        }
